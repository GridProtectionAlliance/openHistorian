...
namespace openHistorian.FileStructure.IO
{
    ...
    unsafe public class PageList : IDisposable
    {
        #region [ Members ]
        ...
        SortedList<int, int> m_pageLookupByPositionIndex;
        ...
        public PageList(BufferPool pool)
        {
            m_pool = pool;
        ...
        public IntPtr GetPointerToPage(int arrayIndex, int incrementReferencedCount)
        {
            if (m_disposed)
                throw new ObjectDisposedException(GetType().FullName);
            var metaData = m_listOfPages.GetValue(arrayIndex);
        ...
        public int DoCollection(int shiftLevel, Func<int, bool> shouldCollect, CollectionEventArgs e)
        {
            ...
            for (int x = 0; x < m_pageLookupByPositionIndex.Count; x++)
            {
                ...
                {
                    var block = m_listOfPages.GetValue(index);
