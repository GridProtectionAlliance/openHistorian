        ...
        protected override void DecodeNextRecord(ref ulong curKey1, ref ulong curKey2, ref ulong curValue1, ref ulong curValue2)
        {
            ...
            if (code < 0xF0) //The time is the same
            {
                ...
                if (code < 192) //value has extra bits and is zero 1-3 bytes in size
                {
                    ...
                    if (code < 64)
                    {
                        extraBits |= (ulong)Stream.ReadByte() << 2;
                    }
                    else if (code < 128)
                    {
                        extraBits |= (ulong)Stream.ReadUInt16() << 2;
                    }
                    else
                    {
                        extraBits |= (ulong)Stream.ReadUInt24() << 2;
                    }
                ...
                else //value is 0 or 4-8 bytes in size
                {
                    switch ((code >> 3) & 7)
                    {
                        case 0:
                            break;
                        case 1:
                            curValue2 ^= Stream.ReadUInt32();
                            break;
                        case 2:
                            curValue2 ^= Stream.ReadUInt40();
                            break;
                        case 3:
                            curValue2 ^= Stream.ReadUInt48();
                            break;
                        case 4:
                            curValue2 ^= Stream.ReadUInt56();
                            break;
                        case 5:
                            curValue2 ^= Stream.ReadUInt64();
                            break;
                    }
                }
            }
            else //The time has changed.
            {
                ...
                switch ((code >> 1) & 7)
                {
                    case 0:
                        break;
                    case 1:
                        curValue2 ^= Stream.ReadByte();
                        break;
                    case 2:
                        curValue2 ^= Stream.ReadUInt16();
                        break;
                    case 3:
                        curValue2 ^= Stream.ReadUInt24();
                        break;
                    case 4:
                        curValue2 ^= Stream.ReadUInt32();
                        break;
                    ...
                    case 7:
                        curValue2 ^= Stream.ReadUInt48();
                        curValue2 ^= (Stream.Read7BitUInt64() << 48);
                        break;
                }
            }
        }
