...
namespace openHistorian.Adapters.Native
{
    ...
    public class DataListener : Component, ISupportLifecycle, ISupportInitialize, IProvideStatus, IPersistSettings
    {
        ...
        public event EventHandler<EventArgs<IList<IData>>> DataExtracted;
        ...
        /// <summary>
        /// Gets the current data for the specified <paramref name="key"/>.
        /// </summary>
        /// <param name="key">Identifier whose current data is to be retrieved.</param>
        /// <returns><see cref="IData"/> if a match is found; otherwise null.</returns>
        public IData FindData(int key)
        {
            IData currentData = null;
            lock (m_data)
            {
                if (key > 0 && key <= m_data.Count)
                {
                    // Valid id is specified, so we'll lookup it's current data.
                    currentData = m_data[key - 1];
                }
            }
            return currentData;
        }
        ...
        protected virtual void OnDataExtracted(IList<IData> data)
        {
            if (DataExtracted != null)
                DataExtracted(this, new EventArgs<IList<IData>>(data));
        }
        ...
        private void DataInitClient_ReceiveDataComplete(object sender, EventArgs<byte[], int> e)
        {
            ...
            if (state.Key > 0)
            {
                lock (m_data)
                {
                    m_data.Add(new ArchiveData(state.Key, state.CurrentData.Time, state.CurrentData.Value, state.CurrentData.Quality));
                }
            }
            else
            {
