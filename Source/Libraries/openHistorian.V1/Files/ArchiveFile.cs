        ...
        private void WriteToCurrentArchiveFile(IDataPoint[] items)
        {
            ...
            foreach (int pointID in sortedDataPoints.Keys)
            {
                ...
                for (int i = 0; i < sortedDataPoints[pointID].Count; i++)
                {
                    ...
                    if ((DateTime)dataPoint.Time > DateTime.UtcNow.AddMinutes(m_leadTimeTolerance))
                    {
                        ...
                        switch (metadata.GeneralFlags.DataType)
                        {
                            case DataType.Analog:
                                if (dataPoint.Value >= metadata.AnalogFields.HighRange)
                                    dataPoint.Flags = (uint)Quality.UnreasonableHigh;
                                else if (dataPoint.Value >= metadata.AnalogFields.HighAlarm)
                                    dataPoint.Flags = (uint)Quality.ValueAboveHiHiAlarm;
                                else if (dataPoint.Value >= metadata.AnalogFields.HighWarning)
                                    dataPoint.Flags = (uint)Quality.ValueAboveHiAlarm;
                                else if (dataPoint.Value <= metadata.AnalogFields.LowRange)
                                    dataPoint.Flags = (uint)Quality.UnreasonableLow;
                                else if (dataPoint.Value <= metadata.AnalogFields.LowAlarm)
                                    dataPoint.Flags = (uint)Quality.ValueBelowLoLoAlarm;
                                else if (dataPoint.Value <= metadata.AnalogFields.LowWarning)
                                    dataPoint.Flags = (uint)Quality.ValueBelowLoAlarm;
                                else
                                    dataPoint.Flags = (uint)Quality.Good;
                                break;
                            case DataType.Digital:
                                if (dataPoint.Value == metadata.DigitalFields.AlarmState)
                                    dataPoint.Flags = (uint)Quality.LogicalAlarm;
                                else
                                    dataPoint.Flags = (uint)Quality.Good;
                                break;
                        }
                    ...
                    if (dataPoint.Time.CompareTo(state.PreviousData.Time) <= 0)
                    {
                        if (dataPoint.Time == state.PreviousData.Time)
                        {
                            ...
                            if (dataPoint.Value == state.PreviousData.Value && (Quality)dataPoint.Flags == state.PreviousData.Quality)
                                return;
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if (metadata.GeneralFlags.AlarmEnabled)
                        {
                            if (metadata.AlarmFlags.Value.CheckBits(BitExtensions.BitVal((int)state.CurrentData.Quality)))
                            {
                                ...
                                if (delay > 0)
                                {
                                    ...
                                    {
                                        if (((TimeTag)state.CurrentData.Time).Value - first > delay)
                                        {
                                    ...
                                    {
                                        m_delayedAlarmProcessing.Add(state.HistorianID, ((TimeTag)state.CurrentData.Time).Value);
                                    }
                                }
                                else
                                {
                        ...
                        if (m_compressData)
                        {
                            ...
                            if (metadata.CompressionMinTime > 0 && (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds() < metadata.CompressionMinTime)
                            {
                                // CompressionMinTime is in effect.
                                archiveData = false;
                                calculateSlopes = false;
                            }
                            else if (state.CurrentData.Quality != state.ArchivedData.Quality || state.CurrentData.Quality != state.PreviousData.Quality || (metadata.CompressionMaxTime > 0 && (state.PreviousData.Time - state.ArchivedData.Time).ToSeconds() > metadata.CompressionMaxTime))
                            {
                            ...
                            else
                            {
                                ...
                                slope1 = (state.CurrentData.Value - (state.ArchivedData.Value + compressionLimit)) / (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds();
                                slope2 = (state.CurrentData.Value - (state.ArchivedData.Value - compressionLimit)) / (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds();
                                currentSlope = (state.CurrentData.Value - state.ArchivedData.Value) / (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds();
                    ...
                    if (calculateSlopes)
                    {
                        if (state.CurrentData.Time != state.ArchivedData.Time)
                        {
                            state.Slope1 = (state.CurrentData.Value - (state.ArchivedData.Value + compressionLimit)) / (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds();
                            state.Slope2 = (state.CurrentData.Value - (state.ArchivedData.Value - compressionLimit)) / (state.CurrentData.Time - state.ArchivedData.Time).ToSeconds();
                        }
                        else
                        {
                            state.Slope1 = 0;
