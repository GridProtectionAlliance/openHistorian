...
namespace openHistorian.V2.Test
{
    ...
    public class BitMathTest
    {
        ...
        static ulong Next(Random r, ulong maxValue)
        {
            return ((ulong)r.Next() << 32 | (uint)r.Next()) % maxValue;
        }
        static uint Next(Random r, uint maxValue)
        {
        ...
        static ulong NextAbove(Random r, ulong maxValue)
        {
            return ((ulong)r.Next() << 32 | (uint)r.Next()) & ~(maxValue - 1);
        }
        static uint NextAbove(Random r, uint maxValue)
        {
