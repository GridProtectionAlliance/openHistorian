...
{
    [TestFixture]
    class SortedTree256DeltaEncodedTest
    {

        class TestEncoding : SortedTree256DeltaEncoded
        {
                ...
                ulong prevKey1, ulong prevKey2, ulong prevValue1, ulong prevValue2)
            {
                ...
                int length;
                base.Stream.Position = 234;
                base.Stream.Write(before1);
                base.Stream.Write(before2);
                byte[] result = new byte[64];
                fixed (byte* lp = result)
                {
                    length = base.EncodeRecord(lp, key1, key2, value1, value2, prevKey1, prevKey2, prevValue1, prevValue2);
                    base.Stream.Write(result, 0, length);
                    base.Stream.Write(after1);
                    base.Stream.Write(after2);
                }
                base.Stream.Position = 234 + 4 + 4;
                ...
                Assert.AreEqual(base.Stream.Position, 234L + 4 + 4 + length);

                base.Stream.Position = 234 + 4 + 4;
                base.Stream.Position = 234;
                Assert.AreEqual(before1, Stream.ReadInt32());
                Assert.AreEqual(before2, Stream.ReadInt32());

                base.Stream.Position = 234L + 4 + 4 + length;
                Assert.AreEqual(after1, Stream.ReadInt32());
                Assert.AreEqual(after2, Stream.ReadInt32());
            }
