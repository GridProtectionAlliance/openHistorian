...
namespace openHistorian.V2.FileSystem
{
    ...
    public class FileAllocationTableTest
    {
        ...
        public void Test()
        {
            ...
            {
                FileAllocationTable header = new FileAllocationTable(data, OpenMode.Create, AccessMode.ReadWrite);
                ...
                FileAllocationTable header2 = new FileAllocationTable(data, OpenMode.Open, AccessMode.ReadOnly);

                CheckEqual(header2, header);
                header = header2.CloneEditableCopy();
                FileAllocationTable_Accessor.AttachShadow(header).m_snapshotSequenceNumber--;
                CheckEqual(header2, header);
        ...
        {
            if (!AreEqual(FileAllocationTable_Accessor.AttachShadow(RW), FileAllocationTable_Accessor.AttachShadow(RO)))
                throw new Exception();
        }
        ...
        static internal bool AreEqual(FileAllocationTable_Accessor other, FileAllocationTable_Accessor self)
        {
            if (other == null)
                return false;
            ...
            else
            {
                ...
                {
                    FileMetaData file = self.m_files[x];
                    ...
                    {
                        if (fileOther == null) return false;
                        if (!FileMetaDataTest.AreEqual(file,fileOther)) return false;
                    }
                }
            }
