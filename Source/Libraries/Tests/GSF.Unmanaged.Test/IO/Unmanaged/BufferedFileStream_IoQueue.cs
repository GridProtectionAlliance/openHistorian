...
namespace GSF.IO.Unmanaged.Test
{
    ...
    {
        private class MemoryFS : FileStream
        {
            private readonly MemS ms;
        ...
        {
            using (MemoryFS fs = new MemoryFS())
            {
                BufferedFileStream.IoQueue io = new BufferedFileStream.IoQueue(fs, 65536, 4096, null);
                ...
                fixed (byte* lpReverse = tmpReverse, lpForward = tmpForward)
                {
                    for (int cnt = 0; cnt < 10; cnt++)
                    {
                        ...
                        PageMetaDataList.PageMetaData page1 = new PageMetaDataList.PageMetaData()
                        {
                            ArrayIndex = R.Next(),
                        ...
                        PageMetaDataList.PageMetaData page2 = new PageMetaDataList.PageMetaData()
                        {
                            ArrayIndex = R.Next(),
                        ...
                        PageMetaDataList.PageMetaData[] lst = new PageMetaDataList.PageMetaData[] {page1, page2};
                        io.Write(lst, false);
