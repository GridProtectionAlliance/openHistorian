        ...
        public static class LargePageSupport
        {
            ...
            public static void VirtualFree(IntPtr ptr)
            {
                ...
                if (success == false)
                {
                    ...
                    throw new Exception("error freeing large block: " + error);
                }
            }
            ...
            public static IntPtr VirtualAllocLargePages(uint size)
            {
                if (CanAllocateLargePage)
                {
                    return VirtualAlloc(IntPtr.Zero, (UIntPtr)size, AllocationType.Commit | AllocationType.Reserve | AllocationType.LargePages, MemoryProtection.Readwrite);
                }
                return IntPtr.Zero;
            }

            #region [ Large Page Support ]
            ...
            static void SetPrivilegeLockMemoryPrivilege()
            {
                ...
                try
                {
                    ...
                    bool success = OpenProcessToken(currentProcessHandle, TokenAdjustPrivileges | TokenQuery,
                                                    ref tokenHandle);
                    if (success == false)
                    {
                        int error = Marshal.GetLastWin32Error();
