//******************************************************************************************************
//  MemoryPool_CollectionEngine.cs - Gbtc
//
...
namespace GSF.UnmanagedMemory
{
    ...
    public partial class MemoryPool
    {
        internal class CollectionEngine
        {
            ...
            public CollectionEngine(MemoryPool pool)
            {
                m_pool = pool;
            ...
            public void AllocateMoreFreeSpace()
            {
                ...
                for (int x = 0; x < collectionCount; x++)
                {
                    if (m_pool.CurrentAllocatedSize < stopShrinkingLimit)
                        break;
                    CollectionEventArgs eventArgs = new CollectionEventArgs(m_pool.InternalTryReleasePage, BufferPoolCollectionMode.Normal, 0);
