...
namespace Historian.StorageSystem.BlockSorter
{
    class TreeHeader
    {
        ...
        public TreeHeader(BinaryStream stream)
        {
            Load(stream);
        }

        public TreeHeader(BinaryStream stream, int blockSize)
        {
            Stream = stream;
            BlockSize = blockSize;
            MaximumLeafNodeChildren = LeafNode.CalculateMaximumChildren(blockSize);
            MaximumInternalNodeChildren = InternalNode.CalculateMaximumChildren(blockSize);
            NextUnallocatedByte = blockSize;
            RootIndexAddress = LeafNode.CreateEmptyNode(this);
            RootIndexLevel = 0;
            Save(stream);
            Load(stream);
        }
