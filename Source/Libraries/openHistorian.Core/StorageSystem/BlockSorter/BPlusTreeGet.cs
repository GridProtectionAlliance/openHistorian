...
namespace Historian.StorageSystem.BlockSorter
{
    ...
    static class BPlusTreeGet
    {
        ...
        internal static long GetKey(TreeHeader header, uint nodeIndex, byte nodeLevel, long key)
        {
            byte level;
            for (int levelCount = nodeLevel; levelCount > 0; levelCount--)
            {
                ...
                level = header.Stream.ReadByte();
                if (level != levelCount)
                    throw new Exception("Node levels corrupt");
#endif
                nodeIndex = InternalNode.GetNodeIndex(header, key, nodeIndex);
            }
            ...
            if (level != 0)
                throw new Exception("Node levels corrupt");
#endif
            return LeafNode.GetValueAddress(header, key, nodeIndex);
        }
