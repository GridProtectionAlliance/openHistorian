...
namespace openHistorian.Engine
{
        ...
        where TValue : class, ISortedTreeValue<TValue>, new()
    {
        ...
        public class Editor : IDisposable
        {
            ...
            public void ReleaseEditLock(ArchiveTable<TKey, TValue> archive)
            {
                if (m_disposed)
                    throw new ObjectDisposedException(GetType().FullName);
            ...
            /// <returns></returns>
            public bool RenewSnapshot(ArchiveTable<TKey, TValue> archive)
            {
                if (m_disposed)
                    throw new ObjectDisposedException(GetType().FullName);
                ...
                {
                    if (partitions[x].ArchiveTable == archive)
                    {
                        partitions[x] = new ArchiveTableSummary<TKey, TValue>(archive);
                        return true;
                    }
                }
                return false;
            ...
            public void Add(ArchiveTable<TKey, TValue> archive, bool isLocked)
            {
                if (m_disposed)
                    throw new ObjectDisposedException(GetType().FullName);
                ArchiveTableSummary<TKey, TValue> summary = new ArchiveTableSummary<TKey, TValue>(archive);
            ...
            public bool Remove(ArchiveTable<TKey, TValue> archive, out ArchiveListRemovalStatus<TKey, TValue> listRemovalStatus)
            {
                if (m_disposed)
                    throw new ObjectDisposedException(GetType().FullName);
                List<ArchiveTableSummary<TKey, TValue>> partitions = m_collection.m_fileSummaries;
                for (int x = 0; x < partitions.Count; x++)
                {
                    if (partitions[x].ArchiveTable == archive)
                    {
                        listRemovalStatus = new ArchiveListRemovalStatus<TKey, TValue>(partitions[x].ArchiveTable, m_collection);
