...
namespace GSF.SortedTreeStore.Tree
{
        ...
        where TKey : SortedTreeTypeBase<TKey>, new()
    {
        ...
        /// <returns></returns>
        private SortedTreeNodeBase<TKey, SortedTreeUInt32> FindNode(TKey key, int level)
        {
            if (level <= 0)
        ...
        /// <returns></returns>
        private SortedTreeNodeBase<TKey, SortedTreeUInt32> GetNode(int nodeLevel)
        {
        ...
        /// <param name="level"></param>
        public void UpdateValue(TKey key, SortedTreeUInt32 value, byte level)
        {
            if (level <= RootNodeLevel)
            {
        ...
        public void Remove(TKey key, byte level)
        {
            if (level <= RootNodeLevel)
            {
                SortedTreeNodeBase<TKey, SortedTreeUInt32> node = GetNode(level);
        ...
        private void CreateNewRootNode(TKey leafKey, uint leafNodeIndex)
        {
            ...
            SortedTreeNodeBase<TKey, SortedTreeUInt32> rootNode = GetNode(RootNodeLevel);
