...
namespace GSF.SortedTreeStore.Net
{
        ...
        where TValue : SortedTreeTypeBase<TValue>, new()
    {
        ...
        public ServerProcessClientEngine(NetworkBinaryStream netStream, SortedTreeClientBase<TKey, TValue> engine)
        {
            ...
            m_encodingMethod = StreamEncoding.CreateStreamEncoding<TKey, TValue>(SortedTree.FixedSizeNode);
        }
            ...
            while (true)
            {
                ...
                switch (command)
                {
                    case ServerCommand.SetEncodingMethod:
                        try
                        {
                            m_encodingMethod = StreamEncoding.CreateStreamEncoding<TKey, TValue>(new EncodingDefinition(m_stream));
                        }
                        catch
                        {
