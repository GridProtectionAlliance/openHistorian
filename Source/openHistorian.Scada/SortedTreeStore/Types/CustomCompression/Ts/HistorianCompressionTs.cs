...
namespace GSF.SortedTreeStore.Tree.TreeNodes
{
    ...
    /// </summary>
    public unsafe class ScadaCompressionTs
        : EncodedNodeBase<ScadaKey, ScadaValue>
    {

        public ScadaCompressionTs(byte level)
            : base(level, 2)
        {
        }
        ...
        protected override unsafe int EncodeRecord(byte* stream, ScadaKey prevKey, ScadaValue prevValue, ScadaKey currentKey, ScadaValue currentValue)
        {
        ...
        protected override unsafe int DecodeRecord(byte* stream, ScadaKey prevKey, ScadaValue prevValue, ScadaKey currentKey, ScadaValue currentValue)
        {
            int size = 0;
            uint code = stream[0];
            //Compression Stages:
        ...
        public override SortedTreeScannerBase<ScadaKey, ScadaValue> CreateTreeScanner()
        {
            return new ScadaCompressionTsScanner(Level, BlockSize, Stream, SparseIndex.Get);
        }
    }
}