...
System.register(["angular", "app/core/utils/kbn", "jquery", "jquery.flot", "jquery.flot.time", "./lib/perfect-scrollbar.min"], function (_export, _context) {
  ...
  return {
    ...
    execute: function () {
      ...
      angular.module("grafana.directives").directive("scadavisLegend", function (popoverSrv, $timeout) {
        return {
          link: function link(scope, elem) {
            var $container = $('<div class="scadavis-legend__container"></div>');
            ...
            function render() {
              ...
              //  $container.empty();
              //  elem.find(".scadavis-legend").css("padding-top", 0);
              //  return;
              //} else {
              elem.find(".scadavis-legend").css("padding-top", 6);
              //}


              if (firstRender) {
                elem.append($container);
                $container.on("click", ".scadavis-legend-icon", openColorSelector);
                $container.on("click", ".scadavis-legend-alias", toggleSeries);
              ...
              $container.toggleClass("scadavis-legend-table", tableLayout);
              ...
              for (i = 0; i < seriesList.length; i++) {
                ...
                var html = '<div class="scadavis-legend-series';
                if (ctrl.hiddenSeries[seriesData.label]) {
                  html += " scadavis-legend-series-hidden";
                }
                html += '" data-series-index="' + i + '">';
                html += '<span class="scadavis-legend-icon" style="float:none;">';
                html += '<i class="fa fa-minus pointer" style="color:' + seriesData.color + '"></i>';
                html += "</span>";

                html += '<a class="scadavis-legend-alias" style="float:none;">' + seriesData.label + "</a>";

                if (showValues && tableLayout) {
                  var value = seriesData.legendData;
                  if (panel.legend.values) {
                    html += '<div class="scadavis-legend-value">' + ctrl.formatValue(value) + "</div>";
                  }
                  if (total) {
                    var pvalue = (value / total * 100).toFixed(decimal) + "%";
                    html += '<div class="scadavis-legend-value">' + pvalue + "</div>";
                  }
                }
